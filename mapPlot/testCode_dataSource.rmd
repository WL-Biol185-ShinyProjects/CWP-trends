...
Test server code for filtering, counting, and then merging with uscounties@data$region
```{r}
values <- final_join %>%
            filter(XRAY_YEAR >= 2000, XRAY_YEAR <= 2002) %>%
            subset(DISEASE == "CWP") %>%
...
filteredFinal <- filter(final_join, XRAY_YEAR >= 2000, XRAY_YEAR <= 2002)
filteredFinal <- left_join(filteredFinal, values, by = "REGION")
```
...
```{r Save Data}
save(df_join, rapidProg, file = "data/serverData.RData")
```
